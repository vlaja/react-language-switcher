...
<Meta title="Components/SwitcherContext" />
...
```tsx
const { language, languageList, setLanguage } = useSwitcherContext();
```

### SwitcherContext.Consumer

You can also use the SwitcherContext.Consumer component if you prefer that way.
...
<Props
  slot={() => ({
    rows: [
      ...
      {
        name: 'setLanguage',
        description: '(lang: string) => void | undefined',
        type: PropTypes.func,
        required: false,
      },
    ],
